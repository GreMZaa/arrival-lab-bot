...
const PricingToggle = ({ isYearly, onToggle }: { isYearly: boolean; onToggle: () => void }) => (
    <div className="flex justify-center items-center gap-4 mb-8 relative z-10">
        <span className={`text-gray-600 font-medium ${!isYearly ? 'text-black font-extrabold' : ''}`}>Разово</span>
        <motion.button
        ...
        <span className={`text-gray-600 font-medium ${isYearly ? 'text-black font-extrabold' : ''}`}>В рассрочку</span>
        {isYearly && (
            <motion.span
                initial={{ opacity: 0, x: -20 }}
                animate={{ opacity: 1, x: 0 }}
                className="text-green-600 font-extrabold text-sm bg-green-100 border border-green-500 px-2 py-0.5 rounded-md"
            >
                Скидка 20%
            </motion.span>
        )}
...
const PricingCard = ({
    ...
    return (
        ...
        >
            ...
            <motion.div
                className={cn(
                    `absolute -top-4 -right-4 px-4 py-2 min-w-[70px] min-h-[70px]
            ...
            >
                <div className="text-center text-white">
                    ...
                    <div className="text-[9px] font-bold">/{isYearly ? 'мес' : 'разово'}</div>
                </div>
            </motion.div>
